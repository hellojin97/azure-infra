...
GitHub Actions에서 job 결과를 Discord webhook으로 알리는 액션. 본 repo의 [01-notify-on-apply.md](01-notify-on-apply.md)에서 사용하지만, 이 문서는 액션 자체의 **모든 입력 / 동작 규칙 / 함정**을 reference로 정리.
...
- `gh secret set ...` 으로 등록 (interactive prompt 또는 stdin 모드 — `--body 'URL'` 인자는 history 노출 위험)
